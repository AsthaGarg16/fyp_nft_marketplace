...
contract("Core ERC721 tests for FantomNFT", function ([owner, minter, buyer, feeRecipient]) {
    ...
    describe("Canceling Item", function () {
        ...
        it("reverts when item is not listed", async function () {
            await expectRevert(
                this.marketplace.cancelListing(this.nft.address, secondTokenId, { from: owner }),
        ...
        it("reverts when not owning the item", async function () {
            await expectRevert(
                this.marketplace.cancelListing(this.nft.address, firstTokenId, { from: owner }),
                "Not listed Item or not owning the item."
            )
        })

        it("successfully cancel the item", async function () {
            await this.marketplace.cancelListing(this.nft.address, firstTokenId, { from: minter })
        })
    })
    ...
    describe("Updating Item Price", function () {
        ...
        it("reverts when item is not listed", async function () {
            await expectRevert(
                this.marketplace.updateListing(this.nft.address, secondTokenId, newPrice, {
                    from: owner,
                }),
                "Not listed Item or not owning the item."
            )
        ...
        it("reverts when not owning the item", async function () {
            await expectRevert(
                this.marketplace.updateListing(this.nft.address, firstTokenId, newPrice, {
                    from: owner,
                }),
                "Not listed Item or not owning the item."
            )
        ...
        it("successfully update the item", async function () {
            await this.marketplace.updateListing(this.nft.address, firstTokenId, newPrice, {
                from: minter,
            })
        })
    })
    ...
    describe("Buying Item", function () {
        ...
        it("successfully purchase item", async function () {
            ...
            const receipt = await this.marketplace.buyItem(this.nft.address, firstTokenId, {
                from: buyer,
                value: pricePerItem,
            })
